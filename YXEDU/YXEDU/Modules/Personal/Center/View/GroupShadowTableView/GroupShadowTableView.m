...
- (void)setCornerRadius:(CGFloat)radius withShadow:(BOOL)shadow withOpacity:(CGFloat)opacity {
    ...
    if (shadow) {
        self.layer.shadowColor = UIColorOfHex(0xAED7E3).CGColor;
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if ([self.groupShadowDelegate respondsToSelector:@selector(groupShadowTableView:heightForHeaderInSection:)]) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if ([self.groupShadowDataSource respondsToSelector:@selector(groupShadowTableView:viewForHeaderInSection:)]) {
        return [self.groupShadowDataSource groupShadowTableView:self viewForHeaderInSection:section];
    } else {
        return nil;
    }
}
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc]initWithFrame:CGRectInset(self.bounds, 14, 0) style:UITableViewStylePlain];
...
- (void)layoutSubviews {
    ...
    [self.tableView setCornerRadius:8 withShadow:YES withOpacity:1];
}
