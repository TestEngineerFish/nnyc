...
#define  TitleDefaultMinWidth 60
...
//            label.backgroundColor = [UIColor orangeColor];
    ...
    if (_currentSelIndex != NSNotFound) {
...
- (void)labelClick:(UITapGestureRecognizer *)tap
{
    ...
    CGFloat titleW = label.enlageTitleWidth;//[self.titleWidths[index] floatValue];
    CGRect oriFrame = self.indicatorV.frame;
    oriFrame.size.width = titleW;
    oriFrame.origin.x = label.titleOriX;//[self.titleOriXs[index] floatValue];
...
//        _indicatorV.frame = oriFrame;
    ...
    [self fatherVCContentscrollViewDidScrollScale:_currentSelIndex];
