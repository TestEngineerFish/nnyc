    ...
    private func createUI() {
        ...
        for (index, range) in array.enumerated() {
            ...
            if !letter2.isEmpty {
                ...
                if (YXConfigure.shared()?.isShowKeyboard ?? false) && (self.exerciseModel.type == .fillWordAccordingToListen || self.exerciseModel.type == .fillWordAccordingToChinese) && self.exerciseModel.question?.extend?.power == .some(10) {
