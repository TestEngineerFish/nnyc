...
//    gradientLayer.colors = @[(__bridge id)[UIColor colorWithRed:250/255.0 green:170/255.0 blue:10/255.0 alpha:0.8].CGColor,(__bridge id)[UIColor colorWithWhite:1 alpha:0.1].CGColor];
    ...
    gradientLayer.colors = @[(__bridge id)[UIColor colorWithRed:167/255.0 green:199/255.0 blue:226/255.0 alpha:0.4].CGColor,(__bridge id)[UIColor whiteColor].CGColor];
...
- (void)setupCircleViews {
    ...
    for (int i = 0; i < pointArray.count; i ++) {
        ...
        FBYLineGraphColorView *lineGraphColorView = [[FBYLineGraphColorView alloc] initWithCenter:[pointArray[i] CGPointValue] radius:10];
        lineGraphColorView.textColor = [UIColor blackColor];
        lineGraphColorView.tag = i + BASE_TAG_CIRCLEVIEW;
        lineGraphColorView.text = [NSString stringWithFormat:@"%@",self.valueArray[i]];
