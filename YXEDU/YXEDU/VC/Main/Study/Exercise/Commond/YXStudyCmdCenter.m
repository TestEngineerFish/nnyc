...
- (void)enterAnswer:(YXAnswerModel *)answer {
    ...
    if ([answer.type isEqualToString:@"1"]) { // 选图
        ...
        if ([answer.answer isEqualToString:topicModel.answer]) {
            self.answerType = YXAnswerEnglishTranslationChineseRight;
            right = @"1";
            [YXUtils playRight];
        } else {
            if (self.answerType == YXAnswerEnglishTranslationChineseState) {
                self.answerType = YXAnswerEnglishTranslationChineseWrong1;
            } else if (self.answerType == YXAnswerEnglishTranslationChineseWrong1) {
                self.answerType = YXAnswerEnglishTranslationChineseWrong2;
            } else if (self.answerType == YXAnswerEnglishTranslationChineseWrong2) {
                self.answerType = YXAnswerEnglishTranslationChineseWrong3;
            }
            [YXUtils playWrong];
        }
...
- (void)studyAction:(YXActionType)actionType {
    if (actionType == YXActionSelectImageContinue) {
        if (self.answerType == YXAnswerSelectImageWrong3) {
            ...
            // TODO::推送Action
            [self pushNextAction];
        } else if (self.answerType == YXAnswerEnglishTranslationChineseWrong3) {
            self.answerType = YXAnswerEnglishTranslationChineseWrong2; // 英译汉
        } else if (self.answerType == YXAnswerEnglishTranslationChineseRight) {
            // TODO::推送Action
            [self pushNextAction];
...
- (void)setAnswerType:(YXAnswerType)answerType {
    ...
    switch (answerType) {
        ...
        case YXAnswerEnglishTranslationChineseState:
            self.showTipView(@(YXAnswerEnglishTranslationChineseState));
            break;
        case YXAnswerEnglishTranslationChineseWrong1:
            self.showTipView(@(YXAnswerEnglishTranslationChineseWrong1));
            break;
        case YXAnswerEnglishTranslationChineseWrong2:
            self.showTipView(@(YXAnswerEnglishTranslationChineseWrong2));
            break;
        case YXAnswerEnglishTranslationChineseWrong3:
            self.showTipView(@(YXAnswerEnglishTranslationChineseWrong3));
            break;
        case YXAnswerEnglishTranslationChineseRight:
            self.showTipView(@(YXAnswerEnglishTranslationChineseRight));
            break;
