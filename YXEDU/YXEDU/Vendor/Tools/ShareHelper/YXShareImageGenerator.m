...
+ (UIImage *)generateGameResultImage:(YXGameResultModel *)model platform:(YXSharePalform)platform {
    NSString *picUrl = [[YXConfigure shared].confModel.baseConfig shareLinkOf:platform];
    CGSize size = CGSizeMake(375, 667);
    UIGraphicsBeginImageContextWithOptions(size, NO, 0.0);
    
    UIBezierPath *bgPath = [UIBezierPath bezierPathWithRect:CGRectMake(0, 0, size.width, size.height)];
    [[UIColor whiteColor] setFill];
    [bgPath fill];
    
    UIImage *gameResultHeader = [UIImage imageNamed:@"gameResultHeader"];
    [gameResultHeader drawInRect:CGRectMake(0, 0, size.width, 164)];
    
    UIImage *userIconbgImage = [UIImage imageNamed:@"userIconbgImage"];
    CGRect userIconBGRect = CGRectMake(64, 78, 91, 91);
    [userIconbgImage drawInRect:userIconBGRect];

    UIImage *newUserIcon = [model.userIcon makeRoundedRadius:0.5f];
    [newUserIcon drawInRect:CGRectInset(userIconBGRect, 12.5, 12.5)];
    
    
    UIImage *gameResultBottom = [UIImage imageNamed:@"gameResultBottom"];
    [gameResultBottom drawInRect:CGRectMake(0, size.height- 147, size.width, 147)];
    
    UIImage *gameSolgan = [UIImage imageNamed:@"gameSolgan"];
    [gameSolgan drawInRect:CGRectMake(17, 183, 317, 31)];
    
    
    CGFloat lrMargin = 12;
    UIImage *gameResultContent = [UIImage imageNamed:@"gameResultContent"];
    [gameResultContent drawInRect:CGRectMake(lrMargin, 234, 200, 254)];
    
    //  detail
    NSMutableParagraphStyle *paragraph = [[NSMutableParagraphStyle alloc] init];
    paragraph.alignment = NSTextAlignmentCenter;
    NSDictionary *attriteDic = @{
                                 NSForegroundColorAttributeName : [UIColor whiteColor],
                                 NSFontAttributeName : [UIFont pfSCSemiboldFontWithSize:15],
                                 NSParagraphStyleAttributeName : paragraph
                                 };
    UIEdgeInsets textInsert = UIEdgeInsetsMake(5, 55, 0, 15);
    
    CGFloat markIconWidth = 138;
    CGFloat markIconHeight = 57;
    CGFloat markVerMargin = 23;
    CGRect markRect = CGRectMake(size.width - lrMargin - markIconWidth, 256, markIconWidth, markIconHeight);
    UIImage *gameResultScore = [UIImage imageNamed:@"gameResultScore"];
    [gameResultScore drawInRect:markRect];
    
    NSString *recordString = [NSString stringWithFormat:@"成绩\n%@",model.rightNum];
    NSMutableAttributedString *attriteStr = [[NSMutableAttributedString alloc] initWithString:recordString attributes:attriteDic];
    NSRange range = [recordString rangeOfString:model.rightNum];
    [attriteStr addAttributes:@{NSFontAttributeName : [UIFont pfSCSemiboldFontWithSize:22]} range:range];
    [attriteStr drawInRect:UIEdgeInsetsInsetRect(markRect, textInsert)];
    
    markRect.origin.y += markVerMargin + markIconHeight;
    UIImage *gameResultSpendTime = [UIImage imageNamed:@"gameResultSpendTime"];
    [gameResultSpendTime drawInRect:markRect];
    
    
    NSString *timeString = [NSString stringWithFormat:@"耗时\n%@",model.answerTimeString];
    attriteStr = [[NSMutableAttributedString alloc] initWithString:timeString attributes:attriteDic];
    range = [timeString rangeOfString:model.answerTimeString];
    [attriteStr addAttributes:@{NSFontAttributeName : [UIFont pfSCSemiboldFontWithSize:16]} range:range];
    [attriteStr drawInRect:UIEdgeInsetsInsetRect(markRect, UIEdgeInsetsMake(7, 55, 5, 15))];
    
    markRect.origin.y += markVerMargin + markIconHeight;
    UIImage *GameResultRanking = [UIImage imageNamed:@"GameResultRanking"];
    [GameResultRanking drawInRect:markRect];
    
    NSString *rankingString = [NSString stringWithFormat:@"排名\n%@",model.rankingString];
    attriteStr = [[NSMutableAttributedString alloc] initWithString:rankingString attributes:attriteDic];
    if (model.ranking) {
        range = [rankingString rangeOfString:model.rankingString];
        [attriteStr addAttributes:@{NSFontAttributeName : [UIFont pfSCSemiboldFontWithSize:22]} range:range];
    }
    ...
    // qrcode
    UIImage *scanqrCode = [UIImage imageNamed:@"scanqrCode"];
    CGRect scanSolganRect = CGRectMake(size.width - 12 - 148, CGRectGetMaxY(markRect) + 40, 148, 43);
    [scanqrCode drawInRect:scanSolganRect];
    
    CGRect qrCodeBgRect = CGRectMake(size.width - 12 - 100, CGRectGetMaxY(scanSolganRect), 100, 100);
    UIBezierPath *qrCodeBGPath = [UIBezierPath bezierPathWithRoundedRect:qrCodeBgRect cornerRadius:3];
    [UIColorOfHex(0xF9B64B) setFill];
    [qrCodeBGPath fill];
    
    if (picUrl) {
        UIImage *qrcode = [self generatorQRCodeImageWith:picUrl withSize:94];
        CGRect qrcodeRect = CGRectInset(qrCodeBgRect, 3, 3);
        [qrcode drawInRect:qrcodeRect];
        UIImage *icon = [UIImage imageNamed:@"miniIcon"];
        [icon drawInRect:CGRectInset(qrcodeRect, 37, 37)];
        
        UIImage *checkFrog = [UIImage imageNamed:@"checkFrog"];
        CGRect checkFrogRect = CGRectMake(CGRectGetMinX(qrcodeRect) - 36, CGRectGetMidY(qrcodeRect) - 25, 36, 50);
        [checkFrog drawInRect:checkFrogRect];
    }
