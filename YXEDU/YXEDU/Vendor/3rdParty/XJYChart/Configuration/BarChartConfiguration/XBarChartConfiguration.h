//
//  XBarChartConfiguration.h
//  XJYChart
//
//  Created by 谢俊逸 on 24/5/2018.
//  Copyright © 2018 JunyiXie. All rights reserved.
//

#import "XJYChart.h"

@interface XBarChartConfiguration : XBaseChartConfiguration

/// defalut is auto calculate x width
@property (nonatomic, assign) CGFloat x_width;
@property (nonatomic, assign) UIColor *backgroundLayerColor;
@property (nonatomic) BOOL isShowDataNumber;
@property (nonatomic, strong) NSArray *selectedColor;
@end
