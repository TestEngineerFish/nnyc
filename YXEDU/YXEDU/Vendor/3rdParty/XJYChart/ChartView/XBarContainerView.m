...
- (void)strokeChart {
    ...
    [fillRectArray enumerateObjectsUsingBlock:^(NSValue* _Nonnull obj,
        ...
        if (self.configuration.isShowDataNumber) {
...
(NSMutableArray<NSValue*>*)rectArray {
    ...
    [rectArray enumerateObjectsUsingBlock:^(NSValue* _Nonnull obj, NSUInteger idx,
        ...
        [self rectShapeLayerWithBounds:rect fillColor:self.configuration.backgroundLayerColor];
...
- (CAGradientLayer*)rectCoverGradientLayerWithBounds:(CGRect)rect {
    ...
    if (self.configuration.selectedColor) {
        gradientLayer.colors = self.configuration.selectedColor;
    } else {
...
- (void)touchesBegan:(NSSet<UITouch*>*)touches withEvent:(UIEvent*)event {
    ...
    [self.backgroundLayerArray enumerateObjectsUsingBlock:^(CALayer* _Nonnull obj,
        ...
        if (CGRectContainsPoint(layerFrame, point)) {
            ...
            [self selectedBarItemWithIndex:idx];
            return;
        }
    }];
}

- (void)selectedBarItemWithIndex:(NSUInteger)idx {
    //上一次点击的layer,清空上一次的状态
    if (self.coverLayer.selectIdxNumber) {
        if (self.coverLayer.selectIdxNumber.intValue < self.layerArray.count) {
