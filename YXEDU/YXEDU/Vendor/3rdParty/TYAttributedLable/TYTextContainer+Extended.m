...
- (void)addWordWithText:(NSString *)text range:(NSRange )range
{
    YXWordTextStorage *wordTextStorage = [[YXWordTextStorage alloc]init];
    wordTextStorage.range = range;
    wordTextStorage.textColor = UIColor.blackColor;
    wordTextStorage.text = text;
    [self addTextStorage:wordTextStorage];
}

#pragma mark - appendWord
- (void)appendWordWithText:(NSString *)wordText
{
    YXWordTextStorage *wordTextStorage = [[YXWordTextStorage alloc] init];
    wordTextStorage.text = wordText;
    wordTextStorage.textColor = UIColor.orangeColor;
    wordTextStorage.font = [UIFont systemFontOfSize:40.f];
    [self appendTextStorage:wordTextStorage];
}

#pragma mark - addSentence

- (void)addSentenceWithText:(NSString *)text range:(NSRange )range
{
    YXSentenceTextStorage *sentenceTextStorage = [[YXSentenceTextStorage alloc]init];
    sentenceTextStorage.range = range;
    sentenceTextStorage.textColor = UIColor.orangeColor;;
    sentenceTextStorage.text = text;
    [self addTextStorage:sentenceTextStorage];
}

#pragma mark - appendSentence
- (void)appendSentenceWithText:(NSString *)sentenceText
{
    YXSentenceTextStorage *sentenceTextStorage = [[YXSentenceTextStorage alloc] init];
    sentenceTextStorage.text = sentenceText;
    sentenceTextStorage.textColor = UIColor.blackColor;
    sentenceTextStorage.font = [UIFont systemFontOfSize:20.f];
    [self appendTextStorage:sentenceTextStorage];
}
