...
@property (nonatomic, strong) NSDictionary  *sentenceRectDictionary; // sentenceRect字典
@property (nonatomic, strong) NSDictionary  *wordRectDictionary;     // wordRect字典
...
- (void)resetRectDictionary
{
    ...
    _sentenceRectDictionary = nil;
    _wordRectDictionary     = nil;
...
//        [_textStorageArray reverseObjectEnumerator];
...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    NSMutableDictionary *wordRectDictionary     = [NSMutableDictionary dictionary];
    NSMutableDictionary *sentenceRectDictionary = [NSMutableDictionary dictionary];
    // 获取每行有多少run
    for (int i = 0; i < numberOfLines; i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            id<TYTextStorageProtocol> textStorageClick = [attributes objectForKey:kTYTextClickRunAttributedName];
            ...
            if (textStorageClick) {
                ...
                if ([textStorageClick conformsToProtocol:@protocol(YXSentenceStorageProtocol)]) {
                    [sentenceRectDictionary setObject:textStorageClick forKey:[NSValue valueWithCGRect:runRect]];
                } else if ([textStorageClick conformsToProtocol:@protocol(YXWordStorageProtocol)]) {
                    [wordRectDictionary setObject:textStorageClick forKey:[NSValue valueWithCGRect:runRect]];
                }
                
                [runRectDictionary setObject:textStorageClick forKey:[NSValue valueWithCGRect:runRect]];
            }
            
            id<TYTextStorageProtocol> textStorageLongPress = [attributes objectForKey:kTYTextLongPressRunAttributedName];
            
            if (textStorageLongPress) {
                CGFloat runWidth  = CTRunGetTypographicBounds(run, CFRangeMake(0,0), &runAscent, &runDescent, NULL);
                ...
                if ([textStorageLongPress conformsToProtocol:@protocol(YXWordStorageProtocol)]) {
                    [wordRectDictionary setObject:textStorageLongPress forKey:[NSValue valueWithCGRect:runRect]];
                } else if ([textStorageLongPress conformsToProtocol:@protocol(YXSentenceStorageProtocol)]) {
                    [sentenceRectDictionary setObject:textStorageLongPress forKey:[NSValue valueWithCGRect:runRect]];
                }
                
                [runRectDictionary setObject:textStorageLongPress forKey:[NSValue valueWithCGRect:runRect]];
            }
        }
    }
    ...
    if (wordRectDictionary.count > 0) {
        _wordRectDictionary = [wordRectDictionary copy];
    } else {
        _wordRectDictionary = nil;
    }
    
    if (sentenceRectDictionary.count > 0) {
        _sentenceRectDictionary = [sentenceRectDictionary copy];
    } else {
        _sentenceRectDictionary = nil;
    }
}
...
{
    return [self enumerateSentenceRect:_linkRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:successBlock];
}

//查找点击位置
- (BOOL)enumerateSentenceRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(YXSentenceTextStorage<YXSentenceStorageProtocol> *textStorage))successBlock
{
    return [self enumerateSentenceRect:_sentenceRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:successBlock];
}

//查找长按位置
- (BOOL)enumerateWordRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(YXWordTextStorage<YXWordStorageProtocol> *textStorage))successBlock
{
    return [self enumerateWordRect:_wordRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:successBlock];
}

//查找点击位置是否在响应范围
- (BOOL)enumerateSentenceRect:(NSDictionary *)sentenceRectDic ContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<TYTextStorageProtocol> textStorage))successBlock
{
    if (sentenceRectDic.count == 0) {
        return NO;
    }
    ...
    // 遍历run位置字典
    [sentenceRectDic enumerateKeysAndObjectsUsingBlock:^(NSValue *keyRectValue, id<TYTextStorageProtocol> textStorage, BOOL *stop) {
        
        CGRect imgRect = [keyRectValue CGRectValue];
        CGRect rect = CGRectApplyAffineTransform(imgRect, transform);
        
        if ([textStorage conformsToProtocol:@protocol(YXSentenceStorageProtocol) ]) {
            // point 是否在rect里
...
//查找长按位置是否在响应范围
- (BOOL)enumerateWordRect:(NSDictionary *)wordRectDic ContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<TYTextStorageProtocol> textStorage))successBlock
{
    if (wordRectDic.count == 0) {
        return NO;
    }
    // CoreText context coordinates are the opposite to UIKit so we flip the bounds
    CGAffineTransform transform = CGAffineTransformScale(CGAffineTransformMakeTranslation(0, viewHeight), 1.f, -1.f);
    ...
    __block BOOL find = NO;
    // 遍历run位置字典
    [wordRectDic enumerateKeysAndObjectsUsingBlock:^(NSValue *keyRectValue, id<TYTextStorageProtocol> textStorage, BOOL *stop) {
        
        CGRect imgRect = [keyRectValue CGRectValue];
        CGRect rect = CGRectApplyAffineTransform(imgRect, transform);
        
        if ([textStorage conformsToProtocol:@protocol(YXWordStorageProtocol) ]) {
            // point 是否在rect里
            if(CGRectContainsPoint(rect, point)){
                find = YES;
                *stop = YES;
