...
public extension UIImage {
    ...
    private class YYIconFont: NSObject {
        ...
        convenience init(code:String, name:String, fontSize:CGFloat, color:UIColor){
            ...
            let rect = nscode.boundingRect(with:CGSize(width: 0.0, height: 0.0) , options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : UIFont.iconfont(size: fontSize)!], context: nil)
            let size = rect.size
            let label = UILabel(frame: CGRect(x: 0, y: 0, width: size.width, height: size.height))
            label.font =  UIFont.iconfont(size: fontSize)
    ...
    private convenience init?(text: Iconfont, fontSize: CGFloat, imageSize: CGSize = CGSize.zero, imageColor: UIColor = UIColor.black) {
        guard let iconfont = UIFont.iconfont(size: fontSize) else {
            self.init()
            return nil
        }
